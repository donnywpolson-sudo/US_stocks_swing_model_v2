...
def load_three_class_trial_contract(repo_root: Path | None = None) -> dict[str, Any]:
    ...
    if (
        ...
        or payload.get("registration") != {"trial_write_authorized": False, "real_history_execution_authorized": False, "required_evidence_class": "REGISTERED_HISTORICAL_DISCOVERY"}
    ):
...
def build_three_class_trial_plan(feature_wfa_plan: Mapping[str, Any], *, repo_root: Path | None = None) -> dict[str, Any]:
    ...
    if feature_wfa_plan.get("wfa") != {"state": "CHRONOLOGICAL_PROXY_PREREGISTRATION_PLANNED_NOT_REGISTERED", "outer_protocol": "rolling_origin", "purge_sessions": 5, "embargo_sessions": 5, "fold_local_transforms_required": True, "trial_registration_required": True, "real_history_execution_authorized": False, "training_or_evaluation_authorized": False}:
