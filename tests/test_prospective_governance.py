...
def test_preregistration_template_uses_local_git_without_writing_or_pushing() -> None:
    ...
    assert template["external_registry"]["status"] == "CONFIGURED_LOCAL_GIT"
    assert template["external_registry"]["registration_available"] is True
    assert template["external_registry"]["owner_controlled"] is True
    assert template["external_registry"]["independent_immutability"] is False
